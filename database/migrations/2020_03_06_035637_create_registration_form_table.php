...
class CreateRegistrationFormTable extends Migration {
	...
	public function up()
	{
		Schema::create('registration_form', function(Blueprint $table)
		{
			...
			$table->text('bed_cost', 65535)->comment('床位费(床位号、房间标准、收费标准)');
			$table->text('meal_cost', 65535)->comment('膳食费(类型、套餐名称、收费标准)');
			$table->text('one-time_cost', 65535)->comment('一次性费(类型、项目名称、收费标准)');
			$table->text('business_change_reason', 65535);
